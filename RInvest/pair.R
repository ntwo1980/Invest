backtest <- function(x, y, ma.period, v.ma.period, vl, w, lp, hp, crash.period = 15, crash.return = -5) {
    ...
    d <- t(apply(dF, 1, function(daily.deal) {
        ...
        last.vma <- ifelse(!is.null(last.daily.deal), last.daily.deal["vma"], 0)
...
df.count <- nrow(df)
df.month <- df[(df.count - 60):df.count,]
plot(df.month$v)
lines(df.month$lp, col = "red")
lines(df.month$hp, col = "blue")
lines(df.month$vma, col = "purple")